...
from lora_mgpo import LoraMGPOTrainer, resolve_mgpo_hparams
import logging
import wandb
...
def train_text_to_text_model(
    run_name: str, # be careful, for llama expriments, run_name should includes "llama" to not use compute_metrics
    ...
    **kwargs,
    ...
    perturbation_method = (perturbation_method or "none").lower()
    mgpo_hparams = None
    if perturbation_method in ("mgpo", "lora_mgpo", "lora-mgpo"):
        TrainerClass = LoraMGPOTrainer
        perturbation_method = "mgpo"
        mgpo_hparams = resolve_mgpo_hparams(
            kwargs.get("mgpo_profile", "nlu"),
            rho=kwargs.get("mgpo_rho", None) if rho <= 0 else rho,
            mu=kwargs.get("mgpo_mu", None),
            beta=kwargs.get("mgpo_beta", None),
        )
        rho = mgpo_hparams["rho"]
    elif rho > 0:
        raise ValueError("rho is only used with perturbation_method=mgpo.")
    if use_loraplus:
        additional_kwargs = {
    ...
    training_args = TrainingArgumentsClass(
        ...
        adam_beta1=(
            mgpo_hparams["mu"]
            if perturbation_method == "mgpo"
            else kwargs.get("adam_beta1", 0.9)
        ),
        **additional_kwargs,
    )

    trainer_kwargs = {"rho": rho}
    if perturbation_method == "mgpo":
        log.info(
            "Using LoRA-MGPO profile=%s rho=%s mu=%s beta=%s",
            mgpo_hparams["profile"],
            mgpo_hparams["rho"],
            mgpo_hparams["mu"],
            mgpo_hparams["beta"],
        )
        trainer_kwargs.update(
            {
                "mgpo_beta": mgpo_hparams["beta"],
                "mgpo_eps": kwargs.get("mgpo_eps", 1e-12),
                "mgpo_normalize": kwargs.get("mgpo_normalize", True),
                "mgpo_log_steps": kwargs.get("mgpo_log_steps", 50),
                "mgpo_profile": mgpo_hparams["profile"],
            }
        )
