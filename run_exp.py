...
def run_exp(cfg: DictConfig):
    ...
    config = {
        ...
        "perturbation_method": getattr(cfg, "perturbation_method", "none"),
        "mgpo_profile": getattr(cfg, "mgpo_profile", "nlu"),
        "mgpo_mu": getattr(cfg, "mgpo_mu", None),
        "mgpo_beta": getattr(cfg, "mgpo_beta", None),
        "mgpo_normalize": getattr(cfg, "mgpo_normalize", True),
    }
    if cfg.wandb.name:
        name = cfg.wandb.name
    else:
    ...
    model = training_loop(
        ...
        mgpo_mu=getattr(cfg, "mgpo_mu", None),
        mgpo_beta=getattr(cfg, "mgpo_beta", None),
        mgpo_eps=getattr(cfg, "mgpo_eps", 1e-12),
        mgpo_normalize=getattr(cfg, "mgpo_normalize", True),
        mgpo_log_steps=getattr(cfg, "mgpo_log_steps", 50),
    )
